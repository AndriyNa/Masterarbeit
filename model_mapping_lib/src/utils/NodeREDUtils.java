...
public class NodeREDUtils {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static JSONObject generateInputNode(TSituationTemplate situationTemplate, JSONObject debugNode,
		...
		// TODO
		TSituation tsituation = situationTemplate.getSituation();
		for (TContextNode sensorNode : tsituation.getContextNode()) {
			connections
					.add(sensorMapping.getObjects() + "." + situationTemplate.getName() + "." + sensorNode.getName());
		}
	...
	public static void deleteFlow(String id) {
		try {
			URL url = new URL(Properties.getProtocol() + "://" + Properties.getServer() + ":" + Properties.getPort()
					+ "/flow/" + id);
			System.out.println(Properties.getProtocol() + "://" + Properties.getServer() + ":" + Properties.getPort()
					+ "/flow/" + id);
	...
	@SuppressWarnings("unchecked")
	public static JSONObject createBroker (){
			JSONObject nodeREDNode = new JSONObject();
			nodeREDNode.put("id", generateNodeREDId());
			nodeREDNode.put("type", "mqtt-broker");
			nodeREDNode.put("z", "");
			nodeREDNode.put("broker", "localhost");
			nodeREDNode.put("port", 1883);
			nodeREDNode.put("clientid", "");
			nodeREDNode.put("usetls", false);
			nodeREDNode.put("compatmode", true);
			nodeREDNode.put("keepalive", "60");
			nodeREDNode.put("cleansession", true);
			nodeREDNode.put("willTopic", "");
			nodeREDNode.put("willQos", "0");
			nodeREDNode.put("willPayload", "");
			nodeREDNode.put("birthTopic", "");
			nodeREDNode.put("birthQos", "0");
			nodeREDNode.put("birthPayload", "");
